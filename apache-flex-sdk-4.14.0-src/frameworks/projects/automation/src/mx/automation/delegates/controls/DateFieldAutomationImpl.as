	...
	public class DateFieldAutomationImpl extends ComboBaseAutomationImpl 
	{
		...
		override public function replayAutomatableEvent(event:Event):Boolean
		{
			...
			var textReplayer:IAutomationObject = 
				dateField.getTextInput() as IAutomationObject;
			...
			if (event is KeyboardEvent)
			{
				...
				if (dateField.getTextInput() && dateField.editable)
				{
					if (keyEvent.keyCode == Keyboard.UP ||
						keyEvent.keyCode == Keyboard.DOWN ||
						keyEvent.keyCode == Keyboard.RIGHT ||
						keyEvent.keyCode == Keyboard.LEFT ||
						...
						return help.replayKeyboardEvent(dateField, KeyboardEvent(event));
					else
			...
			if (dateField.getTextInput() && (event is TextEvent || event is TextSelectionEvent) )
			{
				return textReplayer.replayAutomatableEvent(event);
			}
		...
		private function openCloseHandler(ev:DropdownEvent):void 
		{
			...
			if (ev.triggerEvent)
			{
				var textInput:DisplayObject = dateField.getTextInput() as DisplayObject;
